...
#set document(title: "Sichere nachrichtenbasierte Architekturen mit Python", author: "Philipp Haindl")
...
#set page(paper: "a4", margin: (x: 2.2cm, top: 2.4cm, bottom: 2.2cm),
  header: context {
    ...
    grid(columns: (1fr, auto), [Sichere Messaging-Architekturen],
...
#page(header: none, footer: none, margin: (x: 2.2cm, y: 2.4cm))[
  ...
  #v(0.2cm)
  #text(size: 36pt, weight: "bold", hyphenate: false)[Sichere Messaging-Architekturen]
  #v(0.1cm)
  #text(size: 14pt, fill: c-dark.lighten(20%), hyphenate: false)[Nachrichtenbasierte Systeme mit Python, NATS JetStream und RabbitMQ: Zustellgarantien, Idempotenz, Rechte und signierte Nachrichten]
  #v(1.5cm)
  #align(center, cetz.canvas(length: 1cm, {
    import cetz.draw: *
    kasten((0, 0), [`shop` \ #text(size: 6.5pt)[Producer]], w: 2.2, h: 1.0, bg: rgb("#EAF1FB"), col: c-blue)
    rahmen((3.3, -1.3), (9.3, 1.3), [Broker], c-teal)
    for i in range(5) {
      rect((4.0 + i * 0.95, -0.35), (4.75 + i * 0.95, 0.35), radius: 0.06, fill: c-yellow.lighten(85%), stroke: c-yellow + 0.8pt)
      content((4.375 + i * 0.95, 0), text(size: 6pt, [#(5 - i)]))
    }
    kasten((12.2, 0.8), [`provisionierung`], w: 2.8, h: 0.8, bg: rgb("#FDF1EC"), col: c-accent, size: 7.3pt)
    kasten((12.2, -0.8), [`benachrichtigung`], w: 2.8, h: 0.8, bg: rgb("#FDF1EC"), col: c-accent, size: 7.3pt)
    pfeil((1.15, 0), (3.85, 0), label: "signiert", loff: (0, 0.22))
    pfeil((8.95, 0.15), (10.75, 0.8)); pfeil((8.95, -0.15), (10.75, -0.8))
    content((6.3, -1.75), text(size: 7pt, fill: c-grey.darken(20%), style: "italic")[TLS, minimale Rechte, at-least-once, idempotent verarbeitet])
  }))
  #v(1fr)
  #line(length: 100%, stroke: 0.6pt + c-dark)
  #grid(columns: (1fr, auto), text(size: 9pt)[Für Philipp Haindl \ Python 3.13 · nats-py · aio-pika · NATS 2.x · RabbitMQ 4.x], align(right, text(size: 9pt)[Stand: September 2026]))
]
...
#include "kap01-warum.typ"
#include "kap02-garantien.typ"
#include "kap03-broker.typ"
#metadata("Teil II · Mit Python") <teil>
#include "kap04-nats.typ"
#include "kap05-rabbitmq.typ"
#include "kap06-nachrichten.typ"
#include "kap07-zuverlaessigkeit.typ"
#metadata("Teil III · Sicherheit und Betrieb") <teil>
#include "kap08-transport.typ"
#include "kap09-rechte.typ"
#include "kap10-signatur.typ"
#include "kap11-betrieb.typ"
